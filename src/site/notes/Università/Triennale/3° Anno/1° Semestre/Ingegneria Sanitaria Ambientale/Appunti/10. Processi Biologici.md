...
Per trovare le equazioni di progetto per i [[#Sistema a fanghi attivati - senza ricircolo]] occorre prima di tutto fare i [[#Bilanci per Substrato e Biomassa|bilanci di materia]] per [[#Bilancio del substrato - senza ricircolo|substrato]] e [[#Bilancio della Biomassa - senza ricircolo|Biomassa]]. In entrambi i casi si eseguirà il bilancio supponendo le **condizioni stazionarie**.
...
$$
dove:
- $r_{su}=$ [[#Velocità di utilizzazione del Substrato]]
- Accumulo supposto uguale a zero per via delle **condizioni stazionarie**

Isolando per $r_{SU}$ si avrà quindi che si può scrivere la [[#Velocità di utilizzazione del Substrato]] come
$$
...
$$
Ricordando inoltre l'[[#Equazione di Monod]]: $\mu = \dfrac{\mu_{MAX}S}{k_{s}+S}$
possiamo scrivere:
...
$$
Si noti che l'espressione è uguale a quella ottenuta per il [[#Bilancio del Substrato - senza ricircolo|sistema senza ricircolo]].
...
Si rappresentano gli andamenti delle Equazioni di progetto, sia per il Substrato $S$ che per la Biomassa $X$ in funzione del [[#Tempo di residenza cellulare]] $\theta_{c}$.
...
A valore bassi di $\theta_{c}$ infatti, secondo la definizione di [[#Tempo di residenza cellulare]], corrispondono **bassi valori di ricircolo della biomassa**, e quindi elevati valori di spurgo. Ci sarà pertanto poca biomassa che non basterà a consumare il substrato nel reattore.
...
\mu' = \frac{1}{\theta_{c}}
$$
Si vede che, più la biomassa risiede all'interno del [[#Sistema a fanghi attivati - con ricircolo]], più cresce lentamente, essendo $\mu'$ il [[#Tasso di crescita netta della Biomassa]].
...
Prendo un certo valore di $\theta_{c} = \color{orange}\theta_{c}^{*}$. A questo valore di tempo di residenza cellulare, corrispondono varie concentrazioni di substrato, $S^*$, Biomassa, $X^{*}$ e [[#Tasso di crescita lorda della Biomassa]] (dall'[[#Equazione di Monod]]), $\mu^{*}$.

SI può anche calcolare il [[#Tasso di crescita netta della Biomassa]], $\mu'^{*}$ a questo valore di $\theta_{c}$
$$
\mu'^{*}= \mu^{*} - k_{d}
$$
...
La quantità di Biomassa ($Q_{w}X$) sta al denominatore nell'equazione del [[#Tempo di residenza cellulare]] $\theta_{c}$. Quindi, più aumenta la quantità spurgata, più diminuisce lo il tempo di residenza e viceversa.
...
$$
mentre la quantità $\dfrac{Y}{1+k_{d}\theta_{c}}$ si definisce come [[#Rendimento effettivo di crescita della biomassa]].
...
E quindi, ricordando che avevamo scritto la [[#Produzione di Biomassa]] come $P_{X} = Q(S_{0}-S) \dfrac{Y}{1+k_{d}\theta_{c}}$ si avrà:
$$
P_{X} = Q(S_{0}-S) Y'
$$
Per elevati valori di $\theta_{c}$, il substrato raggiunge valori di concentrazione relativamente bassi, di conseguenza si avrà un contributo maggiore di respirazione endogena. In ogni caso, per valori crescenti di $\theta_{c}$, la [[#Produzione di Biomassa]] tende a decrescere:
$$
...
Voglio confrontare il sistema con spurgo dal reattore con [[#Sistema a fanghi attivati - con ricircolo e spurgo dalla linea di ricircolo|quello con spurgo dalla linea di ricircolo]]. Per farlo dovremo imporre che i due sistemi abbiano un [[#Tempo di residenza cellulare]] uguale. Avremo così due sistemi equivalenti.
...
- $FS_{g}=$ Il contributo della [[#Flusso Solido per gravità|gravità]]
- $FS_{u} =$ Il [[#Flusso Solido di Underflow]]
...
Abbiamo detto che il flusso massimo che posso applicare senza incorrere in fenomeni di accumulo nel [[#Sedimentatore Secondario]] è $FS_{app} = FS_{lim}$.
...
\frac{dFS_{TOT}}{dX} = 0
$$
Spezzando $FS_{TOT}$ nei suoi due contributi ([[#Flusso Solido per gravità]] e [[#Flusso Solido di Underflow]]):
$$
...
Nota $FS_{g}$ posso ricavare la [[#Coniugata della retta di underflow]].
...
La gestione del [[#Sedimentatore Secondario]] consiste nel cercare di mantenere sempre le condizioni di **stato stazionario**.  Possiamo pertanto analizzare le diverse condizioni di carico dei [[#Sistema a fanghi attivati - con ricircolo e spurgo dalla linea di ricircolo|sistemi a fanghi attivati con ricircolo e spurgo dalla linea di ricircolo]]: capire cioè quali siano le condizioni stazionarie di funzionamento del sistema, nonché identificare l'evoluzione nel passaggio da una condizione operativa di stato stazionario a un'altra.
...
Rimarranno invece invariate le condizioni relative alla [[#Velocità di Underflow|retta du Underflow]] e il valore della concentrazione di biomassa $X$ nel reattore (in accordo con la seconda ipotesi)
...
Il [[#Flusso di Solidi]] applicato sarà pertanto variato da $FS_{app}$ a $FS_{app}'$
$$
...
Questa cosa risulta evidenziata anche dal fatto che la [[#Velocità di Underflow|retta di Underflow]] interseca la curva di [[#Flusso Solido per gravità]] $FS_{g}$ in 3 punti anziché 2.
...
A questo punto viene individuato un **nuovo punto di stato**, $P_{S}'$, che sarà ancora definito come l'intersezione tra la nuova [[#Velocità di Overflow|retta di Overflow]] e l'originale [[#Velocità di Underflow|retta di Underflow]].
...
X \to X' \qquad X'>X
$$
Rimangono invariate la pendenza della retta di Underflow ($u = \rm cost$) e la [[#Velocità di Overflow]] $u_{0}$.
...
Come conseguenza si avrà una traslazione verso l'alto della [[#Velocità di Underflow|retta di Underflow]].

Notiamo che, come nel caso di [[#Variazione della portata in ingresso]], in questa condizione la retta di underflow interseca la curva di [[#Flusso Solido per gravità]] $FS_{g}$ in 3 punti individuando una $FS_{app}'>FS_{app}$. Si ha quindi una condizione di **SOVRACCARICO**.
